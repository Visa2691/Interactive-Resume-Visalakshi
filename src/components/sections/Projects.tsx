...
function SmartMealsPreview() {
  return (
    <div className="w-full rounded-xl overflow-hidden bg-slate-950 border border-slate-700/60 shadow-2xl text-[11px] font-mono select-none">
      {/* App header */}
      <div className="flex items-center gap-2 px-3 py-2 bg-slate-900 border-b border-slate-800">
        <div className="w-5 h-5 rounded bg-sky-500/20 border border-sky-500/30 flex items-center justify-center">
          <ChefHat size={10} className="text-sky-400" />
        </div>
        <span className="text-white font-semibold text-[10px]">Smart<span className="text-sky-400">Meals</span></span>
        <span className="ml-auto text-slate-600 border border-slate-700 rounded px-1">multi-agent</span>
      </div>
      {/* Baby age row */}
      <div className="px-3 pt-3 pb-2">
        ...
        <div className="mt-1.5 px-2 py-1.5 rounded-lg bg-sky-500/10 border border-sky-500/20">
          <p className="text-sky-400 font-semibold text-[9px]">8–10 months (Mashed)</p>
          <p className="text-slate-500 text-[8px] mt-0.5">Mashed and soft lumpy textures, 2–3 meals/day</p>
        </div>
      </div>
      {/* Agent pipeline */}
      <div className="px-3 pb-3 space-y-1.5">
        <p className="text-slate-600 text-[8px] uppercase tracking-widest mb-1">Agent pipeline</p>
        {[
          ...
          <div key={a.name} className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-slate-800/80 border border-slate-700/50">
            <span>{a.icon}</span>
  ...
  return (
    <Section id="projects" className="bg-slate-50 dark:bg-slate-950">
      ...
      <div className="grid sm:grid-cols-2 gap-6 items-start">
        ...
        {/* ── SmartMeals — antigravity featured card ── */}
        <motion.div
        ...
        {/* ── FamilyTrip — standard card ── */}
        <motion.div
          initial={{ opacity: 0, y: 24 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5, delay: 0.1 }}
          className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 hover:shadow-md transition-all duration-200 flex flex-col overflow-hidden">
          <div className="p-6 pb-4 border-b border-slate-100 dark:border-slate-700">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="font-display font-bold text-slate-900 dark:text-white text-lg">{familyTrip.title}</h3>
          ...
          <div className="p-6 pt-4 flex-1 flex flex-col">
            <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed mb-5">{familyTrip.description}</p>
